...
-- get a time window that covers the transactions file
SELECT MIN(PURCHASE_DATE) AS FIRST_DATE, MAX(PURCHASE_DATE) AS LAST_DATE FROM TRANSACTIONS;

-- Get generation breakdown of all sales which can be tied to a user
-- Removed category restrictions
SELECT GENERATION, N, SALES_TOTAL, SALES_TOTAL/SUM(SALES_TOTAL) OVER() AS SALES_PCT
FROM(
SELECT CASE
 WHEN EXTRACT(YEAR FROM CAST(U.BIRTH_DATE AS DATE)) >= 1901 AND EXTRACT(YEAR FROM CAST(U.BIRTH_DATE AS DATE)) <= 1927 THEN '0_GREATEST'
...
-- Breakdown of all users by generation
-- Possibly impacted by false birthdays
SELECT GENERATION, N, N/SUM(N) OVER() AS USER_PCT
FROM(
SELECT CASE
 WHEN EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) >= 1901 AND EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) <= 1927 THEN '0_GREATEST'
 WHEN EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) >= 1928 AND EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) <= 1945 THEN '1_SILENT'
 WHEN EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) >= 1946 AND EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) <= 1964 THEN '2_BABY_BOOMER'
 WHEN EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) >= 1965 AND EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) <= 1980 THEN '3_GEN_X'
 WHEN EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) >= 1981 AND EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) <= 1996 THEN '4_MILLENNIAL'
 WHEN EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) >= 1997 AND EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) <= 2010 THEN '5_GEN_Z'
 WHEN EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) >= 2011 AND EXTRACT(YEAR FROM CAST(BIRTH_DATE AS DATE)) <= 2024 THEN '6_GEN_ALPHA'
 ELSE 'LIKELY_ERROR'
END AS GENERATION,
COUNT(*) AS N
FROM USERS
